...
from hp_checker import check_hp_serial
import json

app = Flask(__name__)

# Plantilla HTML para el formulario y resultados
HTML_TEMPLATE = '''
<!doctype html>
<html lang="es">
<head>
  ...
  <title>Consulta de Garantía HP</title>
...
<body>
  <div class="container">
    <div class="card shadow-sm">
      <h3 class="text-center mb-4">Consulta de Garantía HP</h3>
      <form method="post">
...
def index():
    ...
    if request.method == 'POST':
        ...
        if serial_number:
            resultado_json_str = check_hp_serial(serial_number)
...
@app.route('/consultar_hp', methods=['GET'])
def consultar_api():
    """
    Endpoint de la API para consultas de HP.
    """
    serial_number = request.args.get('serie')
    if not serial_number:
        return jsonify({"error": "El parámetro 'serie' es requerido."}), 400
    
    result_json_str = check_hp_serial(serial_number)
    result_data = json.loads(result_json_str)
    return jsonify(result_data)

if __name__ == '__main__':
    # Escucha en todas las interfaces de red
    app.run(host='0.0.0.0', port=5001, debug=True) # Usando el puerto 5001 para no chocar con la otra app
